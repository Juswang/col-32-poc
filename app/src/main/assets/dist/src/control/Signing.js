...
function addPassportToToolbar(passport, i) {
    ...
    w2ui['document-signing-bar'].get('passport').items.push({ text: name, id: 'item ' + (i + 1), value: passport.uuid });
...
L.Map.include({
    ...
    initializeLibrary: function () {
        ...
        if (vereignURL.length == 0)
            return;
        _map = this;
        setupViamAPI('signdocument-iframe-content', {
        ...
        }, getVereignIFrameURL(), getVereignApiURL(), getVereignWopiURL()).then(function (lib) {
            library = lib;
            adjustUIState();
        });
    },

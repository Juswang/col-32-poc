...
L.Clipboard = L.Class.extend({
    ...
    _dataTransferDownloadAndPasteAsync: function (src, dest, fallbackHtml) {
        ...
        that._doAsyncDownload('GET', src, null, false, function (response) {
            window.app.console.log('download done - response ' + response);
            var formData = new FormData();
            formData.append('data', response, 'clipboard');
            that._doAsyncDownload('POST', dest, formData, false, function () {
        ...
        }, function (progress) { return progress / 2; }, function () {
            ...
            }, function (progress) { return 50 + progress / 2; }, function () {
    ...
    dataTransferToDocumentFallback: function (dataTransfer, htmlText, usePasteKeyEvent) {
        ...
        if (content != null) {
            ...
            var that = this;
            this._doAsyncDownload('POST', this.getMetaURL(), formData, false, function () {
    ...
    setTextSelectionText: function (text) {
        this._selectionType = 'text';
        this._selectionContent = this._originWrapBody('<body>' + text + '</body>');
        this._scheduleHideDownload(15);
    },

...
L.Map = L.Evented.extend({
    ...
    setDesktopCalcViewOnZoom: function (zoom, animate) {
        ...
        var topLeftCell = sheetGeom.getCellFromPos(cssBounds.getTopLeft().multiplyBy(app.dpiScale), 'corepixels');
        ...
        var cursorInBounds = calcLayer._cursorCorePixels ?
            cssBounds.contains(L.point(calcLayer._cursorCorePixels.getTopLeft().divideBy(app.dpiScale))) : false;
        ...
        if (animate) {
            this._docLayer.runZoomAnimation(zoom, newPinchCenterLatLng, mapUpdater, runAtFinish);
        }
        else {
            mapUpdater();
            runAtFinish();

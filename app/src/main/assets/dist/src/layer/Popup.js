...
L.Popup = L.Layer.extend({
    ...
    _initLayout: function () {
        var prefix = 'leaflet-popup', container = this._container = L.DomUtil.create('div', prefix + ' ' + (this.options.className || '') +
            ' leaflet-zoom-hide');
    ...
    _updatePosition: function () {
        ...
        }, this._map, new L.Point(offset.x - Math.round(this._containerWidth / 2), -this._container.offsetHeight - offset.y));
        var pos = posVis.position;
        offset = offset.add(pos);
        var rtlAdjust = this._map._docLayer.isCalcRTL();
        var bottom = this._containerBottom = -offset.y, left = this._containerLeft = -Math.round(rtlAdjust ? this._containerWidth : this._containerWidth / 2) + offset.x;

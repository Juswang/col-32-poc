...
L.TileSectionManager = L.Class.extend({
    ...
    _addGridSection: function () {
        ...
        this._sectionContainer.createSection({
            ...
            size: [0, 0],
            expand: '',
            processingOrder: L.CSections.CalcGrid.processingOrder,
    ...
    _addOverlaySection: function () {
        ...
        this._sectionContainer.createSection({
            ...
            onResize: canvasOverlay.onResize.bind(canvasOverlay),
    ...
    _drawGridSectionArea: function (repaintArea, paneTopLeft, canvasCtx) {
        ...
        for (var i = 0; i < ctx.paneBoundsList.length; ++i) {
            ...
            for (var miny = repaintArea.min.y; miny < repaintArea.max.y; miny += bandSize) {
                ...
                context.beginPath();
                // vertical lines
                this.sectionProperties.docLayer.sheetGeometry._columns.forEachInCorePixelRange(repaintArea.min.x, repaintArea.max.x, function (pos) {
                ...
                // horizontal lines
                this.sectionProperties.docLayer.sheetGeometry._rows.forEachInCorePixelRange(miny, maxy, function (pos) {
                    context.moveTo(xTransform(Math.floor(scale * (repaintArea.min.x - paneOffset.x)) + 0.5), Math.floor(scale * (pos - paneOffset.y)) - 0.5);
                    context.lineTo(xTransform(Math.floor(scale * (repaintArea.max.x - paneOffset.x)) - 0.5), Math.floor(scale * (pos - paneOffset.y)) - 0.5);
                    context.stroke();
                });
                context.closePath();
            }
    ...
    _addTilePixelGridSection: function () {
        ...
        this._sectionContainer.createSection({
            ...
            size: [0, 0],
            expand: '',
            processingOrder: L.CSections.Debug.TilePixelGrid.processingOrder,
    ...
    _getZoomDocPos: function (pinchCenter, paneBounds, splitPos, scale, findFreePaneCenter) {
        ...
        var docTopLeft = new L.Point(Math.max(xMin, center.x - (center.x - paneBounds.min.x) / scale), Math.max(yMin, center.y - (center.y - paneBounds.min.y) / scale));
        if (!findFreePaneCenter)
            return { topLeft: docTopLeft };
        // Assumes paneBounds is the bounds of the free pane.
        var paneSize = paneBounds.getSize();
        var newPaneCenter = new L.Point((docTopLeft.x - splitPos.x + (paneSize.x + splitPos.x) / (2 * scale)) * scale / app.dpiScale, (docTopLeft.y - splitPos.y + (paneSize.y + splitPos.y) / (2 * scale)) * scale / app.dpiScale);
        return {
            topLeft: docTopLeft,
...
L.CanvasTileLayer = L.Layer.extend({
    ...
    _retainParent: function (x, y, z, part, minZoom) {
        var x2 = Math.floor(x / 1.2), y2 = Math.floor(y / 1.2), z2 = z - 1;
        var key = x2 + ':' + y2 + ':' + z2 + ':' + part, tile = this._tiles[key];
        if (tile && tile.active) {
            tile.retain = true;
            return true;
        }
    ...
    _reset: function (center, zoom, hard, noPrune, noUpdate) {
        var tileZoom = Math.round(zoom), tileZoomChanged = this._tileZoom !== tileZoom;
    ...
    _extractAndSetGraphicSelection: function (messageJSON) {
        ...
        if (hasGridOffset) {
            ...
            this._graphicSelectionTwips = this._getGraphicSelectionRectangle(new L.Bounds(topLeftTwips, bottomRightTwips));
        }
        this._graphicSelection = new L.LatLngBounds(this._twipsToLatLng(this._graphicSelectionTwips.getTopLeft(), this._map.getZoom()), this._twipsToLatLng(this._graphicSelectionTwips.getBottomRight(), this._map.getZoom()));
    ...
    _onGraphicViewSelectionMsg: function (textMsg) {
        ...
        if (strTwips != null) {
            ...
            var offset = new L.Point(parseInt(strTwips[2]), parseInt(strTwips[3]));
            var bottomRightTwips = topLeftTwips.add(offset);
            var boundRectTwips = this._getGraphicSelectionRectangle(new L.Bounds(topLeftTwips, bottomRightTwips));
            this._graphicViewMarkers[viewId].bounds = new L.LatLngBounds(this._twipsToLatLng(boundRectTwips.getTopLeft(), this._map.getZoom()), this._twipsToLatLng(boundRectTwips.getBottomRight(), this._map.getZoom()));
        }
        else {
    ...
    _onCellCursorMsg: function (textMsg) {
        ...
        if (textMsg.match('EMPTY') || !this._map.isPermissionEdit()) {
            ...
            var offset = new L.Point(parseInt(strTwips[2]), parseInt(strTwips[3]));
            var bottomRightTwips = topLeftTwips.add(offset);
            this._cellCursorTwips = this._convertToTileTwipsSheetArea(new L.Bounds(topLeftTwips, bottomRightTwips));
            this._cellCursor = new L.LatLngBounds(this._twipsToLatLng(this._cellCursorTwips.getTopLeft(), this._map.getZoom()), this._twipsToLatLng(this._cellCursorTwips.getBottomRight(), this._map.getZoom()));
    ...
    _showURLPopUp: function (position, url) {
        ...
        if (!url.startsWith('#')) {
            ...
            if (offsetDiffTop < 10)
                this._movePopUpBelow();
            if (offsetDiffLeft < 10)
                this._movePopUpRight();
            var map_ = this._map;
    ...
    _onInvalidateCursorMsg: function (textMsg) {
        ...
        this._visibleCursor = new L.LatLngBounds(this._twipsToLatLng(recCursor.getTopLeft(), this._map.getZoom()), this._twipsToLatLng(recCursor.getBottomRight(), this._map.getZoom()));
    ...
    _onInvalidateViewCursorMsg: function (textMsg) {
        ...
        this._viewCursors[viewId].bounds = new L.LatLngBounds(this._twipsToLatLng(rectangle.getTopLeft(), this._map.getZoom()), this._twipsToLatLng(rectangle.getBottomRight(), this._map.getZoom())),
    ...
    _onCellViewCursorMsg: function (textMsg) {
        ...
        if (obj.rectangle.match('EMPTY')) {
            ...
            this._cellViewCursors[viewId].bounds = new L.LatLngBounds(this._twipsToLatLng(boundsTwips.getTopLeft(), this._map.getZoom()), this._twipsToLatLng(boundsTwips.getBottomRight(), this._map.getZoom()));
    ...
    _onTextSelectionEndMsg: function (textMsg) {
        ...
        if (rectangles.length && this._map.isPermissionEdit()) {
            ...
            this._textSelectionEnd = new L.LatLngBounds(this._twipsToLatLng(topLeftTwips, this._map.getZoom()), this._twipsToLatLng(bottomRightTwips, this._map.getZoom()));
    ...
    _onCellSelectionAreaMsg: function (textMsg) {
        ...
        if (strTwips != null && this._map.isPermissionEdit()) {
            ...
            this._cellSelectionArea = new L.LatLngBounds(this._twipsToLatLng(boundsTwips.getTopLeft(), this._map.getZoom()), this._twipsToLatLng(boundsTwips.getBottomRight(), this._map.getZoom()));
    ...
    _onGraphicMove: function (e) {
        ...
        if (e.type === 'graphicmovestart') {
            ...
            if (dragConstraint) {
                ...
                this._graphicSelectionTwips.min.x + deltaPos.x, this._graphicSelectionTwips.min.y + deltaPos.y);
    ...
    _onSelectionHandleDrag: function (e) {
        if (e.type === 'drag') {
            ...
            var containerPos = new L.Point(expectedPos.x - this._map._container.getBoundingClientRect().left, expectedPos.y - this._map._container.getBoundingClientRect().top);
    ...
    _sendReferenceRangeCommand: function (startCol, startRow, endCol, endRow) {
        this._map.sendUnoCommand('.uno:CurrentFormulaRange?StartCol=' + startCol +
            '&StartRow=' + startRow +
            '&EndCol=' + endCol +
            '&EndRow=' + endRow +
    ...
    _onUpdateCellCursor: function (horizontalDirection, verticalDirection, onPgUpDn) {
        ...
        if (this._cellCursor && !this._isEmptyRectangle(this._cellCursor)) {
            ...
            if (this._cellCursorMarker) {
                ...
                this._cellCursorMarker = new CCellCursor(corePxBounds, {
                    name: 'cell-cursor',
                    pointerEvents: 'none',
                    fill: false,
    ...
    getPaneLatLngRectangles: function () {
        ...
        return paneRects.map(function (pxBound) {
            return new L.LatLngBounds(map.unproject(pxBound.getTopLeft().divideBy(app.dpiScale)), map.unproject(pxBound.getBottomRight().divideBy(app.dpiScale)));
        });
    },
    ...
    _debugAddInvalidationRectangle: function (topLeftTwips, bottomRightTwips, command) {
        var now = +new Date();
        var invalidBoundCoords = new L.LatLngBounds(this._twipsToLatLng(topLeftTwips, this._tileZoom), this._twipsToLatLng(bottomRightTwips, this._tileZoom));
    ...
    _syncTileContainerSize: function () {
        ...
        if (tileContainer) {
            ...
            if (!heightIncreased && isTabletOrMobile && this._map._docLoaded && hasVisibleCursor) {
                ...
                var cursorPositionInView = centerOffset.x > -viewHalf.x && centerOffset.x < viewHalf.x &&
    ...
    _wrapCoords: function (coords) {
        return new L.TileCoordData(this._wrapX ? L.Util.wrapNum(coords.x, this._wrapX) : coords.x, this._wrapY ? L.Util.wrapNum(coords.y, this._wrapY) : coords.y, coords.z, coords.part);
    },
    ...
    _pxBoundsToTileRange: function (bounds) {
        return new L.Bounds(bounds.min.divideBy(this._tileSize).floor(), bounds.max.divideBy(this._tileSize).floor());
    },
    ...
    _cssBoundsToCore: function (bounds) {
        return new L.Bounds(this._cssPixelsToCore(bounds.min), this._cssPixelsToCore(bounds.max));
    },
    _twipsToCorePixels: function (twips) {
        return new L.Point(twips.x / this._tileWidthTwips * this._tileSize, twips.y / this._tileHeightTwips * this._tileSize);
    },
    ...
    _corePixelsToTwips: function (corePixels) {
        return new L.Point(corePixels.x / this._tileSize * this._tileWidthTwips, corePixels.y / this._tileSize * this._tileHeightTwips);
    },
    _twipsToCssPixels: function (twips) {
        return new L.Point((twips.x / this._tileWidthTwips) * (this._tileSize / app.dpiScale), (twips.y / this._tileHeightTwips) * (this._tileSize / app.dpiScale));
    },
    _cssPixelsToTwips: function (pixels) {
        return new L.Point(((pixels.x * app.dpiScale) / this._tileSize) * this._tileWidthTwips, ((pixels.y * app.dpiScale) / this._tileSize) * this._tileHeightTwips);
    },
    ...
    _latLngToCorePixels: function (latLng, zoom) {
        var pixels = this._map.project(latLng, zoom);
        return new L.Point(pixels.x * app.dpiScale, pixels.y * app.dpiScale);
    },
    ...
    _twipsToCoords: function (twips) {
        return new L.TileCoordData(Math.round(twips.x / twips.tileWidth) * this._tileSize, Math.round(twips.y / twips.tileHeight) * this._tileSize);
    },
    _coordsToTwips: function (coords) {
        return new L.Point(Math.floor(coords.x / this._tileSize) * this._tileWidthTwips, Math.floor(coords.y / this._tileSize) * this._tileHeightTwips);
    },
        ...
        for (var rangeIdx = 0; rangeIdx < tileRanges.length; ++rangeIdx) {
            ...
            for (var j = tileRange.min.y; j <= tileRange.max.y; ++j) {
                for (var i = tileRange.min.x; i <= tileRange.max.x; ++i) {
                    var coords = new L.TileCoordData(i * this._tileSize, j * this._tileSize, zoom, this._selectedPart);
    ...
    _updateOnChangePart: function () {
        ...
        for (var rangeIdx = 0; rangeIdx < tileRanges.length; ++rangeIdx) {
            ...
            for (var j = tileRange.min.y; j <= tileRange.max.y; j++) {
                for (var i = tileRange.min.x; i <= tileRange.max.x; i++) {
                    coords = new L.TileCoordData(i * this._tileSize, j * this._tileSize, zoom, this._selectedPart);
                    if (!this._isValidTile(coords)) {
                        continue;
                    }
                    key = this._tileCoordsToKey(coords);
    ...
    updateHorizPaneSplitter: function () {
        ...
        if (!this._xSplitter) {
            this._xSplitter = new CSplitterLine(map, {
                ...
                thickness: Math.round(this._splittersStyleData.getFloatPropWithoutUnit('border-top-width')
                    * app.dpiScale),
                isHoriz: true
            });
    ...
    _coordsToTileBounds: function (coords) {
        ...
        var tileTopLeft = new L.Point(coords.x * this.options.tileWidthTwips / this._tileSize / zoomFactor, coords.y * this.options.tileHeightTwips / this._tileSize / zoomFactor);
...
L.TilesPreFetcher = L.Class.extend({
    ...
    preFetchTiles: function (forceBorderCalc, immediate) {
        ...
        var finalQueue = [];
        var visitedTiles = {};
        var validTileRange = new L.Bounds(new L.Point(0, 0), new L.Point(Math.floor((this._docLayer._docWidthTwips - 1) / this._docLayer._tileWidthTwips), Math.floor((this._docLayer._docHeightTwips - 1) / this._docLayer._tileHeightTwips)));
            ...
            while (tilesToFetch > 0 && paneBorder.getBorderIndex() < maxBorderWidth) {
                ...
                if (fetchBottomBorder) {
                    for (var i = clampedBorder.min.x; i <= clampedBorder.max.x; i++) {
                        ...
                        var coords = new L.TileCoordData(i * tileSize, borderBounds.max.y * tileSize);
                        queue.push(coords);
                    }
                }
                if (fetchTopBorder) {
                    for (i = clampedBorder.min.x; i <= clampedBorder.max.x; i++) {
                        ...
                        coords = new L.TileCoordData(i * tileSize, borderBounds.min.y * tileSize);
                        queue.push(coords);
                    }
                }
                if (fetchRightBorder) {
                    for (i = clampedBorder.min.y; i <= clampedBorder.max.y; i++) {
                        ...
                        coords = new L.TileCoordData(borderBounds.max.x * tileSize, i * tileSize);
                        queue.push(coords);
                    }
                }
                if (fetchLeftBorder) {
                    for (i = clampedBorder.min.y; i <= clampedBorder.max.y; i++) {
                        ...
                        coords = new L.TileCoordData(borderBounds.min.x * tileSize, i * tileSize);
                        queue.push(coords);
                    }
                }
        ...
        if (!immediate)
            window.app.console.assert(finalQueue.length <= maxTilesToFetch, 'finalQueue length(' + finalQueue.length + ') exceeded maxTilesToFetch(' + maxTilesToFetch + ')');

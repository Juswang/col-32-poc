...
L.PathTransform.Handle = L.CircleMarker.extend({
    ...
    onAdd: function (map) {
        ...
        if (this.options.setCursor) { // SVG/VML
            this.setCursorType(L.PathTransform.Handle.CursorsByType[this.options.index]);
        }
    }
});
...
L.PathTransform.PolyHandle = L.PathTransform.Handle.extend({
    ...
    onAdd: function (map) {
        L.CircleMarker.prototype.onAdd.call(this, map);
        if (this.options.setCursor) { // SVG/VML
            this.setCursorType(L.PathTransform.Handle.PolyCursorsByType[this.options.cursor.toString()]);
        }
    }
});
L.Handler.PathTransform = L.Handler.extend({
    ...
    */
    ...
    _getPoints: function () {
        ...
        if (rectangleHandles && rectangleHandles !== '') {
            var nw = rectangleHandles['1'][0], north = rectangleHandles['2'][0], ne = rectangleHandles['3'][0], west = rectangleHandles['4'][0], east = rectangleHandles['5'][0], sw = rectangleHandles['6'][0], south = rectangleHandles['7'][0], se = rectangleHandles['8'][0];
    ...
    _createGluePoints: function () {
        ...
        for (var i = 0; i < this.options.shapes.length; i++) {
            ...
            for (var j = 0; j < shape.gluepoints.length; j++) {
                ...
                this._glueHandlers.push(new GlueHandleClass(point, L.Util.extend({}, this.options.gluePointOptions, { ordNum: shape.ordnum }))
    ...
    _createHandlers: function () {
        ...
        if (this.options.scaling) {
            ...
            for (var i = 0; i < points.length; i++) {
                ...
                this._handlers.push(this._createHandler(this._map._docLayer._twipsToLatLng(points[i].point, this._map.getZoom()), i * 2, i, this._onScaleStart)
    ...
    */
    _createRotationHandlers: function () {
        ...
        this._rotationMarker = new RotateHandleClass(handlerPosition, this.options.handlerOptions)
            .addTo(this._handlersGroup)
    ...
    */
    _createPolyHandlers: function () {
        ...
        for (var i = 0; i < handleList.length; ++i) {
            ...
            this._handlers.push(this._createPolyHandler(handleList[i], i)
                .addTo(this._handlersGroup));
        }
    },
    _onPolyHandleDragStart: function (evt) {
        ...
        if (isEdge < 0) {
            ...
            if (marker.options.cursor == 10) {
                bottom = new L.LatLng((latlngs[0].lat + latlngs[3].lat) / 2, middleHandlelatLngs.lng);
                topPoint = new L.LatLng((latlngs[1].lat + latlngs[2].lat) / 2, middleHandlelatLngs.lng);
            }
            else {
                bottom = new L.LatLng(middleHandlelatLngs.lat, (latlngs[2].lng + latlngs[3].lng) / 2);
                topPoint = new L.LatLng(middleHandlelatLngs.lat, (latlngs[0].lng + latlngs[1].lng) / 2);
            }
            this._polyLine = new L.Polyline([topPoint, bottom], this.options.polyLineOptions).addTo(this._handlersGroup);
        }
        else {
    ...
    _createCustomHandlers: function () {
        ...
        for (var i = 0; i < handles.length; ++i) {
            ...
            var marker = new this.options.customHandleClass(customHandlePosition, L.Util.extend({}, this.options.handlerOptions, options))
                .addTo(this._handlersGroup)
                .on('mousedown', this._onCustomHandleDragStart, this);
    ...
    */
    _createHandler: function (latlng, type, index, event) {
        ...
        var marker = new HandleClass(latlng, L.Util.extend({}, this.options.handlerOptions, options));
    ...
    _createPolyHandler: function (handle, index) {
        ...
        var marker = new HandleClass(this._map._docLayer._twipsToLatLng(handle.convertedPoint, this._map.getZoom()), L.Util.extend({}, this.options.handlerOptions, options));
